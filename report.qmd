...
Clearly the data from previous elections helps with the predictions. Which is not really surprising since the population, economic situation and other factors do not change that much over the course of one parlimentary cycle thus there's no reason to behave differently from other similar districts. 
...
Multinomial linear regression is like normal linear regression but with more outputs. The model used in this case is taken from sklearn library, namely `MultiTaskElasticNet`. ElasticNet is a method of regularisation (constraining the learned parameters) which adds terms (to the loss function) that are linear and quadratic in the learned parameters which should help make better models. The model has parameter "L1 Ratio" which sets the ratio of the quadratic and linear penalty terms in the loss function.
...
In conclusion we've found out that eventhough some correlations between sociodemographic indicators and election results were present in the data, the data that is most useful for election prediction are the results from previous elections. 



And what would have helped achieve better results? First of all, more data. One of the best predictors of election results are per capita distraints and debts in given region. Another good predictor would be location of the district since districts close to one another tend to vote similarly. Moreover, spending more time with neural networks to get at least a working model, maybe even try to find some NN architecture in the literature. Skipping the normalization of data or stratification of splits would probably perform worse but it would show how important these steps are. For better evaluation, as mentioned, using KL Divergence would be a good idea however that would require further processing of data.
