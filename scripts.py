...
object_theme = {**sns.axes_style("darkgrid"), **sns.plotting_context("notebook")}
sns.set_palette("Paired")
sns.set_theme(**theme)


def plot_gender_counts(df: pd.DataFrame):
    ax = _plot_dist_bar(df, ["sl11muzi", "sl11zeny"], ["Men", "Women"])
    ax.set_title("Number of men and women")
    return ax


def plot_edu_count(df: pd.DataFrame):
    ax = _plot_dist_bar_ratio(
        df,
    ...
    ax.set_title("Ratio of inhabitants with a particular level of education")


def plot_municipality_size_ratio(df: pd.DataFrame):
    counts = df.groupby("vel.obce_cat")["sl11obyvatel"].sum()[
        ["500-", "500-1k", "1-5k", "5-10k", "10-20k", "20-50k", "50-100k", "100k+"]
    ]
    ax = sns.barplot(x=counts / df["sl11obyvatel"].sum(), y=counts.index)
    ax.set_xlabel("Number of inhabitants")
    ax.set_ylabel("Size of municipality")
    ax.set_title("Ratio of inhabitants living a municipality of given size")
...
def _plot_dist_bar_ratio(df, cols, names, ax=None):
    counts = df[cols].sum()
    ax = sns.barplot(x=counts / df["sl11obyvatel"].sum(), y=names, ax=ax)
    return ax


def _plot_dist_bar(df, cols, names):
    counts = df[cols].sum()
    ax = sns.barplot(x=counts, y=names)
    return ax


def _plot_election_graph(df, cols, names, vote_count_col, ax=None):
    counts = df[cols].sum()[cols]
    return sns.barplot(x=counts / df[vote_count_col].sum(), y=names, ax=ax)


def plot_parl_elections(df: pd.DataFrame):

    (fig, [ax1, ax2]) = plt.subplots(2, 1)
    w, h = fig.get_size_inches()
    fig.set_size_inches(w, h * 2)
    ax1 = plot_elections_2017(df, ax1)
    ax2 = plot_elections_2021(df, ax2)
    return fig, [ax1, ax2]


def plot_elections_2017(df, ax=None):
    ax = _plot_election_graph(
        df,
        [
            "par17ano",
            "par17ods",
...
def box_cross_size_plot(df: pd.DataFrame, base_col, translation):
    df = df.groupby("vel.obce_cat")[list(translation.keys()) + [base_col]].sum()
    ...
    plot = (
        so.Plot(data=df, x="value", y=df.index, color="variable")
        .add(so.Bar(), so.Stack())
        .scale(color="Paired")
    ).theme(object_theme)
...
def create_jointplot(df, x, y, labels):
    fig, axes = plt.subplots(
        ncols=len(x), nrows=len(y), figsize=(len(x) * 8, len(y) * 6)
    )
    for i in range(len(x)):
        for j in range(len(y)):
            (
                so.Plot(df, x=x[i], y=y[j])
